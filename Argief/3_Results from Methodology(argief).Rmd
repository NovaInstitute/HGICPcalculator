...
load("../Rda/Data_from_s2.Rda")
load("../Rda/Tables_for_3.Rda")
load("../Rda/ConMon.Rda")
library(knitr)
library(tidyverse)
library(kableExtra)
...
#source("../R/tydelik_calculateEEFfromRR.R")
```
...
### Number of days and number of subpopulations
...
$n^*_{ij}$: total number of days that household *j* in subpopulation *i* was observed in the CM phase.

$n_{(KT)}$: total number of subpopulations in the KT phase.
...
\[
\overline{x}_{(M)ij} =  \sum_{k}^{n_{(P)ij}} \frac{x_{(M)ijk}}{fr_{(M)ijk}} 
\]
...
```{r}
kable(RR_EEFij %>% select(place, year, household_qr_code, XBij_m3,
...
Table \ref{tab:dfresB_unnest} gives the observations for a single household. Column *XBijk_m2* gives $\frac{x_{(B)ijk}}{fr_{(B)ijk}}$, the daily average fuel use per fire. The average daily average per household is given in column *XBij_m2*.  
...
```{r LE}
kable(LE %>% left_join(dfCy) %>% select(LEi, CPi ) %>%  left_join(dfCOEF) %>% left_join(dffNRB), digits = 2, booktabs = TRUE, caption = "Calculation of LE") %>%  kableExtra::kable_styling(latex_options = c("HOLD_position" ))
```
...
\overline{frr}_i\times \overline{fr^*}_{(M)i} \times \overline{x}_{(B)i})$$

*   $\overline{d}_{(P)i}$ is the average days over the sampled households of project operation in subpopulation *i*.

*   $N_{(P)i}$ is the project population size in subpopulation *i*. At this stage it is set at $N = 1000$.

*   $\overline{frr}_i$ is the average frequency ratio of non project measures fires to the project measures fires in subpopulation *i* in the project scenario of the KT phase.
*   $\overline {fr^*}_{(M)i}$ is the average project measures frequency per day in subpopulation *i* in the post KT phase .

*   $\overline{x}_{(B)i}$ is the average fuel use (in kg) per fire for subpopulation *i* in the baseline scenario of the KT phase. 

```{r}
kable(dfCLi %>%
    head(5),
    digits = 2, booktabs = TRUE,
    caption = "\\label{tab:CLi}Get CLi")%>% 
  kableExtra::kable_styling(latex_options = c("HOLD_position",  "scale_down" ))
```
...
The average frequency ratio of non-project measures fires to the project measures fires in the project scenario for household *j* in subpopulation *i* is calculated as $$\overline{frr}_i = \frac {1}{n_{(P)i}} \sum_{j}^{n_{(P)i}}\frac{fr_{(BP)ij.}}{fr_{(M)ij.}}$$

*   $n_{(P)i}$ is the number of households that were observed in the project phase of the KT.
*   $fr_{(BP)ij.}$ is the total frequency of non-project measures fires for household *j* in subpopulation *i* in the project phase of the KT.
*   $fr_{(M)ij.}$ is the total frequency of project measures fires for household *j* in subpopulation *i* in the project phase of the KT.
...
kable(dfFrRij %>%
    head(5),
    digits = 2, booktabs = TRUE,
    caption = "\\label{tab:FrR}Sample of the calculation of frequency ratio $frr_{ij}$ from the project KT phase")
```
...
The average baseline fuel use per fire (in kg) for subpopulation *i* is calculated as the average of the household average fuel use per fire observed in the baseline phase of the kitchen test.  $$\overline{x}_{(B)i}=\frac{1}{n_{(B)i}}\sum_{j}^{n_{(B)i}} \frac{x_{(B)ij.}}{fr_{(B)ij.}}$$

*   $x_{(B)ij.}$ is the total fuel use for household *j* in subpopulation *i* for the baseline phase of the KT.

*   $fr_{(B)ij}$ is the total fire frequency for household *j* in subpopulation *i* in the baseline phase of the KT.

* $n_{(B)i}$ is the number of households that were observed in the baseline phase of the KT.


```{r}
kable(dfresB %>% select(-data) %>% head() ,
      digits = 2, booktabs = TRUE, 
      caption = "\\label{tab:dfresB}Sample of data from baseline kitchen test results summarised by household") %>% 
  kableExtra::kable_styling(latex_options = c("HOLD_position",  "scale_down" ))
```
