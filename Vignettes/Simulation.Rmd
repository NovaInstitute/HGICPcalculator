...
```{r}
kable(eef, digits = 1, booktabs = TRUE, caption = "Calculation of $eef$ from simulated data") %>% 
...
```{r}
kable(LE %>% left_join(CLy ) %>% select( LE, LP ) %>%  left_join(dfCOEF) %>% left_join(dffNRB), digits = 2, booktabs = TRUE, caption = "Calculation of BE from simulated results") %>%  kableExtra::kable_styling(latex_options = c("HOLD_position" ))
```
